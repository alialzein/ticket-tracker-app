...
export async function renderTickets() {
    ...
    ticketsToRender.forEach((ticket, index) => {
        ...
        if (appState.expandedTicketId) {
            ...
            isCollapsed = true; // Collapse all by default
        }
        ...
        const notesHTML = (ticket.notes || []).map((note, index) => {
            ...
            if (isMyNote) {
                ...
                kudosButtonHtml = `<button 
                    ...
                    ${haveIGivenKudos ? '' : `onclick="event.stopPropagation(); tickets.giveKudos(${ticket.id}, ${index}, '${note.username}')"`}
            ...
            <div class="note-container bg-gray-700/30 p-2 rounded-md border border-gray-600/50 slide-in flex justify-between items-start gap-2">
                ...
                <button onclick="event.stopPropagation(); tickets.deleteNote(${ticket.id}, ${index}, '${note.username}', '${note.user_id || ''}')" class="text-gray-400 hover:text-red-400 transition-colors p-1 opacity-75 hover:opacity-100 flex-shrink-0" title="Delete note">
        ...
        ticketElement.innerHTML = `
                ...
                <div class="flex-grow min-w-0">
                    <div class="flex justify-between items-center mb-1">
                        <p class="text-xs">
                            ...
                            ${ticket.assigned_to_name ? `→ <span class="font-bold text-purple-400">${ticket.assigned_to_name}</span>` : ''}
                        </p>
                 ...
                 <div class="flex items-center gap-2">
                    ...
                    <button class="ticket-collapse-btn p-1 rounded-full hover:bg-gray-700/50">
                        <svg class="w-4 h-4 transition-transform ${isCollapsed ? '' : 'rotate-180'}" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                    </button>
                </div>
            </div>
                    ...
                    <div class="note-container relative">
                        <div id="note-editor-${ticket.id}" class="note-editor"></div>
                         <div class="flex justify-end mt-2">
                            <button onclick="event.stopPropagation(); tickets.addNote(${ticket.id})" class="bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold py-2 px-4 rounded-lg transition-colors hover-scale">Add Note</button>
                        </div>
                    </div>
                </div>
            </div>
            <div class="mt-2 pt-3 border-t border-gray-700/50 flex justify-between items-center" onclick="event.stopPropagation()">
                ...
                <div class="flex justify-end items-center gap-2">
                    <label for="add-attachment-${ticket.id}" class="cursor-pointer text-gray-400 hover:text-indigo-400 p-2 transition-colors hover-scale" title="Add Attachment" onclick="event.stopPropagation();">
                    ...
                    <button onclick="event.stopPropagation(); tickets.toggleFollowUp(${ticket.id}, ${ticket.needs_followup})" title="Toggle Follow-up" class="p-1 rounded-full hover:bg-gray-700/50">
                    ...
                    ${!isAssignedToMe ? `<button onclick="event.stopPropagation(); tickets.assignToMe(${ticket.id})" class="text-gray-400 hover:text-green-400 p-2 transition-colors hover-scale" title="Assign to Me">
                    ...
                    <button onclick="event.stopPropagation(); ui.openEditModal(${ticket.id})" class="text-gray-400 hover:text-indigo-400 p-2 transition-colors hover-scale" title="Edit Ticket">
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16"><path d="M15.502 1.94a.5.5 0 0 1 0 .706L14.459 3.69l-2-2L13.502.646a.5.5 0 0 1 .707 0l1.293 1.293zm-1.75 2.456-2-2L4.939 9.21a.5.5 0 0 0-.121.196l-.805 2.414a.25.25 0 0 0 .316.316l2.414-.805a.5.5 0 0 0 .196-.12l6.813-6.814z"/><path fill-rule="evenodd" d="M1 13.5A1.5 1.5 0 0 0 2.5 15h11a1.5 1.5 0 0 0 1.5-1.5v-6a.5.5 0 0 0-1 0v6a.5.5 0 0 1-.5.5h-11a.5.5 0 0 1-.5-.5v-11a.5.5 0 0 1 .5-.5H9a.5.5 0 0 0 0-1H2.5A1.5 1.5 0 0 0 1 2.5v11z"/></svg>
                    </button>
                    ${ticket.user_id === appState.currentUser.id ? `<button onclick="event.stopPropagation(); tickets.deleteTicket(${ticket.id})" class="text-gray-400 hover:text-red-500 p-2 transition-colors hover-scale" title="Delete Ticket">
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16"><path d="M5.5 5.5A.5.5 0 0 1 6 6v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5zm2.5 0a.5.5 0 0 1 .5.5v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5zm3 .5a.5.5 0 0 0-1 0v6a.5.5 0 0 0 1 0V6z"/><path fill-rule="evenodd" d="M14.5 3a1 1 0 0 1-1 1H13v9a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V4h-.5a1 1 0 0 1-1-1V2a1 1 0 0 1 1-1H6a1 1 0 0 1 1-1h2a1 1 0 0 1 1 1h3.5a1 1 0 0 1 1 1v1zM4.118 4 4 4.059V13a1 1 0 0 0 1 1h6a1 1 0 0 0 1-1V4.059L11.882 4H4.118zM2.5 3V2h11v1h-11z"/></svg>
                    </button>` : ''}
                </div>
            </div>`;
    ...
    ticketsToRender.forEach(ticket => {
        if (document.getElementById(`note-editor-${ticket.id}`)) {
    ...
    // Reset the expanded ticket ID after the render is complete so it doesn't affect subsequent renders.
    if (appState.expandedTicketId) {
        appState.expandedTicketId = null;
    }
}

export function toggleNoteExpansion(ticketId, noteIndex) {
    const noteText = document.getElementById(`note-text-${ticketId}-${noteIndex}`);
    const toggleBtn = document.getElementById(`toggle-btn-${ticketId}-${noteIndex}`);
    if (!noteText || !toggleBtn) return;
    noteText.classList.toggle('collapsed');
